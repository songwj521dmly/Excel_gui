...
### 5.18 新增：许可证管理与激活码生成
- **需求**: 集成类似 CadCad 的激活码逻辑，实现软件授权保护。激活码需基于时间戳生成，并具备 10 分钟有效期验证机制。
- **实现**:
  - **核心服务 (`LicenseManager`)**:
    - **单例模式**: 确保全局唯一的许可证管理实例。
    - **加密算法**: 实现 AES-256-CBC 加密/解密逻辑（基于 Windows CAPI），兼容 C# `LicenseService.cs` 的密钥生成规则（SHA256 派生 Key/IV）。
    - **注册表存储**: 使用 `QSettings` 读写注册表 `HKEY_CURRENT_USER\Software\ExcelCL`，存储加密后的过期时间。
    - **激活逻辑**: 验证激活码格式（Base64 编码的加密时间戳），解密后校验时间是否在当前时间的前 10 分钟内。激活成功后更新注册表过期时间。
  - **测试工具 (`license_test`)**: 创建控制台测试程序，验证激活码生成、校验及注册表读写功能。
