...
template <typename... Block> struct UintData
{
    ...
    copyTo(Result &resultObj, size_t dataoffset, uint &metatypeoffset) const
    {
        uint *ptr = resultObj.staticData.data;
        size_t payloadoffset = dataoffset + headerSize();
        Result *resultPtr = &resultObj;
        data.forEach([&, resultPtr](const auto &input) {
            // copy the uint data
            ...
            // copy the metatypes
            decltype(input.metaTypes())::template copyTo<Unique>(*resultPtr, metatypeoffset);
